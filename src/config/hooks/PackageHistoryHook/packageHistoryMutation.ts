...
export const usePackageHistoryMutations = () => {
    ...
    const createMutation = useApiMutation<IPackagePurchase, PackageHistoryCreatePayload>(
        (data) => PackageHistoryRoute.create(data),
        {
            ...
            onError: (error) => {
                toast.error("Gagal membuat riwayat paket", {
                    description:
                    error.message || "Terjadi kesalahan saat membuat riwayat paket",
                });
            },
        }
    );

    // const updateMutation = useApiMutation<IPackagePurchase, PackageHistoryUpdatePayload>(
    //     (data) => PackageHistoryRoute.update(data),
    //     {
    //         onSuccess: (updatedPackageHistory) => {
    //             queryClient.setQueryData(
    //                 PACKAGE_HISTORY_QUERY_KEYS.detail(updatedPackageHistory.id), updatedPackageHistory
    //             );
    //             invalidatePackageHistoryQueries(queryClient);
    ...
    //                 description:
    //                 error.message || "Terjadi kesalahan saat memperbarui riwayat paket",
    //             });
    //         },
    //     }
    // );
        ...
        (id) => PackageHistoryRoute.remove(id),
        {
            ...
            onError: (error) => {
                toast.error("Gagal menghapus riwayat paket", {
                    description:
                    error.message || "Terjadi kesalahan saat menghapus riwayat paket",
                });
            },
        }
    );
